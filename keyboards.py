...
CANCEL_BUTTON = InlineKeyboardButton(text="✖ Отмена", callback_data="cancel_to_menu")
...
def instructor_card_keyboard(instructor_id: int, index: int, total: int):
    nav_row = []
    if index > 0:
        nav_row.append(InlineKeyboardButton(text="← Назад", callback_data="prev_instructor"))
    if index < total - 1:
        nav_row.append(InlineKeyboardButton(text="Далее →", callback_data="next_instructor"))
...
def days_keyboard(available_days: list, max_weeks_ahead: int, week_offset: int):
    ...
    nav = []
    if week_offset > 0:
        nav.append(InlineKeyboardButton(text="← Пред. неделя", callback_data="prev_week"))
    if week_offset < max_weeks_ahead - 1:
        nav.append(InlineKeyboardButton(text="След. неделя →", callback_data="next_week"))
    if nav:
        rows.append(nav)
...
def main_menu_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Запись на практические занятия", callback_data="start_booking")],
